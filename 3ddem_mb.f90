module constant
    ...
    !粒子のパラメータ
    integer, parameter :: N = 6000, Nstep = 3000000
    ...
    !壁のパラメータ
    real(8), parameter :: Dwall = 30.0d0, W_wall(0:2) = (/0.0d0, 0.0d0, 0.1d0/)
    real(8), parameter :: etan_wall = -2 * log(e) * sqrt(m * kn / ((acos(-1.0d0)) ** 2 + (log(e)) ** 2)), etat_wall = etan_wall

    !円筒容器のパラメータ
    real(8), parameter :: H_bottom = 10.0d0    !原点を中心に左右にH_bottomとる
    ...
    real(8), save :: deltat_prev(0:2,1:N,1:N) = 0
    real(8), save :: torque(0:2) = 0, torque_all(0:2,1:N) = 0
    real(8), save :: deltat_prev_wall(0:2,1:N) = 0.0d0
    real(8), save :: deltat_prev_bottom_wall(0:2,1:N,1:2) = 0.0d0    !1は左の壁，2は右の壁
...
contains
    ...
    subroutine mkvtk_wall(step, dirname)
        integer, intent(in) :: step
        character(len=*), intent(in) :: dirname
        integer i
        character(len=40) str_step
...
contains
    ...
    subroutine cylinder_bottom_wall_contact(i,LorR)
...
program dem_3d
    ...
    do step = 1, Nstep
        ...
        do i = 1, N
            ...
            ! fall(0:2,i) = fall(0:2,i) + (/5.0d0*sin(10.0d0 * step*dt), 0.0d0, 0.0d0/)
